...
namespace Core {
  ...
  QString getInstructionMnemonic(ProcessorVersion version, uint8_t opCode);
  ...
  static const QMap<QString, MnemonicInfo> mnemonics = {
    ...
    {"BEQ", {{0, 0, 0, 0, 0, 0x27}, "------", "Branch if equal (Z=1, signed/unsigned)", "Branches if the zero flag (Z) is set, AdrMode::INDicating equality. Used in both signed and unsigned comparisons.", M6800 | M6803}},
    ...
    {"BMI", {{0, 0, 0, 0, 0, 0x2B}, "------", "Branch if negative (N=1, signed)", "Branches if negative flag (N) is set, AdrMode::INDicating a negative result in signed arithmetic.", M6800 | M6803}},
    {"BNE", {{0, 0, 0, 0, 0, 0x26}, "------", "Branch if not equal (Z=0, signed/unsigned)", "Branches if zero flag (Z) is clear, AdrMode::INDicating inequality. Used in both signed and unsigned comparisons.", M6800 | M6803}},
    {"BPL", {{0, 0, 0, 0, 0, 0x2A}, "------", "Branch if positive (N=0, signed)", "Branches if negative flag (N) is clear, AdrMode::INDicating a positive result in signed arithmetic.", M6800 | M6803}},
    {"BRA", {{0, 0, 0, 0, 0, 0x20}, "------", "Branch always", "Unconditional branch to a AdrMode::RELative address.", M6800 | M6803}},
    ...
    {"CPX",
     {{0, 0x8C, 0x9C, 0xAC, 0xBC, 0},
      "--****",
      "X - M:(M+1)",
      "The more significant byte of the contents of the AdrMode::INDex register is compared with the contents of the byte of memory at the address specified by the program. The less significant byte of the contents of the AdrMode::INDex register is compared with the contents of the nAdrMode::EXT "
      "byte of memory, at one plus the address specified by the program. The Z bit is set or reset according to the results of these comparisons, and may be used subsequently for conditional branching. The N and V bits, though determined by this operation, are not intended for conditional "
      "branching.",
      M6800 | M6803}},
    {"DAA",
     {{0x19, 0, 0, 0, 0, 0},
    ...
    {"DEX", {{0x09, 0, 0, 0, 0, 0}, "---*--", "X <- X - 1", "Subtract one from the AdrMode::INDex register.", M6800 | M6803}},
    {"EORA",
    ...
    {"INX", {{0x08, 0, 0, 0, 0, 0}, "---*--", "X <- X + 1", "Add one to the AdrMode::INDex register.", M6800 | M6803}},
    ...
    {"JSR",
     {{0, 0, 0x9D, 0xAD, 0xBD, 0},
      "------",
      "Jump to subroutine",
      "The program counter is incremented by 3 or by 2, depending on the addressing mode, and is then pushed onto the stack, eight bits at a time. The stack pointer points to the nAdrMode::EXT empty location in the stack. A jump occurs to the instruction stored at the numerical address.",
      M6800 | M6803}},
    ...
    {"LDS",
     {{0, 0x8E, 0x9E, 0xAE, 0xBE, 0},
      "--**0-",
      "SP <- M:(M+1)",
      "Loads the more significant byte of the stack pointer from the byte of memory at the address specified by the program, and loads the less significant byte of the stack pointer from the nAdrMode::EXT byte of memory, at one plus the address specified by the program.",
      M6800 | M6803}},
    {"LDX",
     {{0, 0xCE, 0xDE, 0xEE, 0xFE, 0},
      "--**0-",
      "X <- M:(M+1)",
      "Loads the more significant byte of the AdrMode::INDex register from the byte of memory at the address specified by the program, and loads the less significant byte of the AdrMode::INDex register from the nAdrMode::EXT byte of memory, at one plus the address specified by the program.",
      M6800 | M6803}},
    ...
    {"RTI",
     {{0x3B, 0, 0, 0, 0, 0},
      ...
      "The condition codes, accumulators B and A, the AdrMode::INDex register, and the program counter, will be restored to a state pulled from the stack. Note that the interrupt mask bit will be reset if and only ifthe corresponding bit stored in the stack is zero.",
      M6800 | M6803}},
    {"RTS",
     {{0x39, 0, 0, 0, 0, 0},
      "------",
    ...
    {"STS",
     {{0, 0, 0x9F, 0xAF, 0xBF, 0},
      "--**0-",
      "M:(M+1) <- SP",
      "Stores the more significant byte of the stack pointer in memory at the address specified by the program, and stores the less significant byte of the stack pointer at the nAdrMode::EXT location in memory, at one plus the address specified by the program.",
      M6800 | M6803}},
    {"STX",
     {{0, 0, 0xDF, 0xEF, 0xFF, 0},
      "--**0-",
      "M:(M+1) <- X",
      "Stores the more significant byte of the AdrMode::INDex register in memory at the address specified by the program, and stores the less significant byte of the AdrMode::INDex register at the nAdrMode::EXT location in memory, at one plus the address specified by the program.",
      M6800 | M6803}},
    ...
    {"SWI",
     {{0x3F, 0, 0, 0, 0, 0},
      "-1----",
      "Software Interrupt",
      "The program counter is incremented (by 1). The program counter, AdrMode::INDex register, and accumulator A and B, are pushed into the stack. The condition codes register is then pushed into the stack, with condition codes H, I, N, Z, V, C going respectively into bit positions 5 thru 0, and "
      "the top two bits (in bit positions 7 and 6) are set (to the 1 state). The stack pointer is decremented (by 1) after each byte of data is stored in the stack. The interrupt mask bit is then set. The program counter is then loaded with the address stored in the software interrupt pointer at "
      "memory locations "
    ...
    {"TSX", {{0x30, 0, 0, 0, 0, 0}, "------", "X <- SP + 1", "Loads the AdrMode::INDex register with one plus the contents of the stack pointer. The contents of the stack pointer remain unchanged.", M6800 | M6803}},
    {"TXS", {{0x35, 0, 0, 0, 0, 0}, "------", "SP <- X - 1", "Loads the stack pointer with the contents of the AdrMode::INDex register, minus one. The contents of the AdrMode::INDex register remain unchanged.", M6800 | M6803}},
    {"WAI", {{0x3E, 0, 0, 0, 0, 0}, "------", "Wait for interrupt", "Halt program execution and waits for an interrupt.", M6800 | M6803}},
  };

  const QList<QString> M6800Mnemonics = {"ZERO",
                                         "NOP",
                                         "AdrMode::INVALID",
                                         "AdrMode::INVALID",
                                         "AdrMode::INVALID",
                                         ...
                                         "LDX",
                                         "STX"};
  const QList<QString> M6803Mnemonics = {"ZERO",
                                         "NOP",
                                         "AdrMode::INVALID",
                                         "AdrMode::INVALID",
                                         "LSRD",
