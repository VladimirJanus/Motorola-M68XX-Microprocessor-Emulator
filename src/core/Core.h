...
namespace Core {
  ...
  static const QList<MnemonicInfo> mnemonics = {
    ...
    {"SWI",   {0x3F, 0, 0, 0, 0, 0},          "-1----", "Software Interrupt",                             "The program counter increments by 1. The PC, index register, and accumulators A and B are pushed onto the stack. The condition codes register follows, with H, I, N, Z, V, C in bit positions 5-0, while bits 7-6 are set to 1. The stack pointer decrements after each byte is stored. The interrupt mask bit is set. Finally, the PC loads the address from the software interrupt pointer at memory locations (n-5) and (n-4), where n is the address $FFFF.", M6800 | M6803},
