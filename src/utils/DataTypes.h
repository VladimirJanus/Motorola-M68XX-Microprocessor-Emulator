...
namespace DataTypes {
  ...
  static const QList<Instruction> M6800InstructionPage = {
    {INVALID, 0 },
    {INH,     2 },
    {INVALID, 0 },
    {INVALID, 0 },
  ...
  static const QList<Instruction> M6803InstructionPage = {
    {INVALID, 0 },
    {INH,     2 },
    {INVALID, 0 },
    {INVALID, 0 },
  ...
  static const QMap<QString, Allias> alliasMap = {
    {"BYTE", {".BYTE", "Allias for .BYTE", M6800 | M6803}                              },
  ...
  static const QMap<QString, MnemonicInfo> mnemonics = {
    ...
    {"ANDA",  {{0, 0x84, 0x94, 0xA4, 0xB4, 0}, "--**0-", "ACCA <- ACCA · M", "Performs logical 'AND' between the contents of ACCA and the contents of M and places the result in ACCA. (Each bit of ACCA after the operation will be the logical 'AND' of the corresponding bits of M and of ACCA before the operation.)", M6800 | M6803}                                                                                                                                                                             },
    {"ANDB",  {{0, 0xC4, 0xD4, 0xE4, 0xF4, 0}, "--**0-", "ACCB <- ACCB · M", "Performs logical 'AND' between the contents of ACCB and the contents of M and places the result in ACCB. (Each bit of ACCB after the operation will be the logical 'AND' of the corresponding bits of M and of ACCB before the operation.)", M6800 | M6803}                                                                                                                                                                             },
    ...
    {"CPX",
     {{0, 0x8C, 0x9C, 0xAC, 0xBC, 0},
      ...
      M6800 | M6803}                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   },
    {"DAA",   {{0x19, 0, 0, 0, 0, 0}, "--****", "decimal adjust ACCA", "Adjusts the contents of the ACCA register after arithmetic operations with binary-coded-decimal (BCD) operands. It ensures proper representation of BCD sums by adjusting both high and low nibbles, considering the carry bit's state.", M6800 | M6803}                                                                                                                                                                                       },
    ...
    {"EORA",  {{0, 0x88, 0x98, 0xA8, 0xB8, 0}, "--**0-", "ACCA <- ACCA XOR M", "Perform logical 'EXCLUSIVE OR' between the contents of ACCA and the contents of M, and place the result in ACCA. (Each bit of ACCA afterthe operation will be the logical 'EXCLUSIVE OR' of the corresponding bit of M and ACCA before the operation.)", M6800 | M6803}                                                                                                                                                                },
    {"EORB",  {{0, 0xC8, 0xD8, 0xE8, 0xF8, 0}, "--**0-", "ACCB <- ACCB XOR M", "Perform logical 'EXCLUSIVE OR' between the contents of ACCB and the contents of M, and place the result in ACCB. (Each bit of ACCB afterthe operation will be the logical 'EXCLUSIVE OR' of the corresponding bit of M and ACCB before the operation.)", M6800 | M6803}                                                                                                                                                                },
    ...
    {"JSR",   {{0, 0, 0x9D, 0xAD, 0xBD, 0}, "------", "Jump to subroutine", "The program counter is incremented by 3 or by 2, depending on the addressing mode, and is then pushed onto the stack, eight bits at a time. The stack pointer points to the next empty location in the stack. A jump occurs to the instruction stored at the numerical address.", M6800 | M6803}                                                                                                                                          },
    ...
    {"LDS",   {{0, 0x8E, 0x9E, 0xAE, 0xBE, 0}, "--**0-", "SP <- M:(M+1)", "Loads the more significant byte of the stack pointer from the byte of memory at the address specified by the program, and loads the less significant byte of the stack pointer from the next byte of memory, at one plus the address specified by the program.", M6800 | M6803}                                                                                                                                                             },
    {"LDX",   {{0, 0xCE, 0xDE, 0xEE, 0xFE, 0}, "--**0-", "X <- M:(M+1)", "Loads the more significant byte of the index register from the byte of memory at the address specified by the program, and loads the less significant byte of the index register from the next byte of memory, at one plus the address specified by the program.", M6800 | M6803}                                                                                                                                                            },
    ...
    {"ORAA",  {{0, 0x8A, 0x9A, 0xAA, 0xBA, 0}, "--**0-", "ACCA <- ACCA ∨ M", "Perform logical 'OR' between the contents of ACCA and the contents of M and places the result in ACCA. (Each bit of ACCA afterthe operation will be the logical 'OR' of the corresponding bits of M and of ACCA before the operation).", M6800 | M6803}                                                                                                                                                                                },
    {"ORAB",  {{0, 0xCA, 0xDA, 0xEA, 0xFA, 0}, "--**0-", "ACCB <- ACCB ∨ M", "Perform logical 'OR' between the contents of ACCB and the contents of M and places the result in ACCB. (Each bit of ACCB afterthe operation will be the logical 'OR' of the corresponding bits of M and of ACCB before the operation).", M6800 | M6803}                                                                                                                                                                                },
    ...
    {"PULX",  {{0x38, 0, 0, 0, 0, 0}, "------", "Pull X", "SP is incremented. The data at the address which is contained in the SP is then loaded from the stack to the least significant byte of X register. Then SP is incremented again, and the data at the address of SP is stored to the most significant byte of the X register.", M6803}                                                                                                                                                                       },
    ...
    {"RTI",   {{0x3B, 0, 0, 0, 0, 0}, "******", "Return from interrupt", "The condition codes, accumulators B and A, the index register, and the program counter, will be restored to a state pulled from the stack. Note that the interrupt mask bit will be reset if and only ifthe corresponding bit stored in the stack is zero.", M6800 | M6803}                                                                                                                                                                  },
    {"RTS",   {{0x39, 0, 0, 0, 0, 0}, "------", "Return from subroutine", "The stack pointer is incremented (by 1). The contents of the byte of memory, at the address now contained in the stack pointer, are loaded into the 8 bits of highest significance in the program counter. The stack pointer is again incremented (by 1). The contents of the byte of memory, at the address now contained in the stack pointer, are loaded into the 8 bits of lowest significiance in the program counter.", M6800 | M6803}},
    ...
    {"STS",   {{0, 0, 0x9F, 0xAF, 0xBF, 0}, "--**0-", "M:(M+1) <- SP", "Stores the more significant byte of the stack pointer in memory at the address specified by the program, and stores the less significant byte of the stack pointer at the next location in memory, at one plus the address specified by the program.", M6800 | M6803}                                                                                                                                                                          },
    {"STX",   {{0, 0, 0xDF, 0xEF, 0xFF, 0}, "--**0-", "M:(M+1) <- X", "Stores the more significant byte of the index register in memory at the address specified by the program, and stores the less significant byte of the index register at the next location in memory, at one plus the address specified by the program.", M6800 | M6803}                                                                                                                                                                         },
    ...
    {"SWI",
     {{0x3F, 0, 0, 0, 0, 0},
      "-1----",
      "Software Interrupt",
      "The program counter is incremented (by 1). The program counter, index register, and accumulator A and B, are pushed into the stack. The condition codes register is then pushed into the stack, with condition codes H, I, N, Z, V, C going respectively into bit positions 5 thru 0, and the top two bits (in bit positions 7 and 6) are set (to the 1 state). The stack pointer is decremented (by 1) after each byte of data is stored in the stack. The interrupt mask bit is then set. The program counter is then loaded with the address stored in the software interrupt pointer at memory locations "
      "(n-5) and (n-4), where n is the address corresponding to a high state on all lines of the address bus.",
      M6800 | M6803}                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   },
  ...
  const QList<QString> M6800Mnemonics = {"ZERO", "NOP",     "INVALID", "INVALID", "INVALID", "INVALID", "TAP",     "TPA",     "INX",     "DEX",     "CLV",     "SEV",  "CLC",     "SEC",     "CLI",     "SEI",  "SBA",  "CBA",     "INVALID", "INVALID", "INVALID", "INVALID", "TAB",  "TBA",     "INVALID", "DAA",  "INVALID", "ABA",     "INVALID", "INVALID", "INVALID", "INVALID", "BRA",  "INVALID", "BHI",  "BPL",     "BCC",  "BCS",  "BNE",  "BEQ",     "BVC",  "BVS",  "BPL",     "BMI",     "BGE",     "BLT",     "BGT",     "BLE",  "TSX",     "INS",     "PULA", "PULB",
                                         "DES",  "TCS",     "PSHA",    "PSHB",    "INVALID", "RTS",     "INVALID", "RTI",     "INVALID", "INVALID", "WAI",     "SWI",  "NEGA",    "INVALID", "INVALID", "COMA", "LSRA", "INVALID", "RORA",    "ASRA",    "ASLA",    "ROLA",    "DECA", "INVALID", "INCA",    "TSTA", "INVALID", "CLRA",    "NEGB",    "INVALID", "INVALID", "COMB",    "LSRB", "INVALID", "RORB", "ASRB",    "ASLB", "ROLB", "DECB", "INVALID", "INCB", "TSTB", "INVALID", "CLRB",    "NEG",     "INVALID", "INVALID", "COM",  "LSR",     "INVALID", "ROR",  "ASR",
                                         "ASL",  "ROL",     "DEC",     "INVALID", "INC",     "TST",     "JMP",     "CLR",     "NEG",     "INVALID", "INVALID", "COM",  "LSR",     "INVALID", "ROR",     "ASR",  "ASL",  "ROL",     "DEC",     "INVALID", "INC",     "TST",     "JMP",  "CLR",     "SUBA",    "CMPA", "SBCA",    "INVALID", "ANDA",    "BITA",    "LDAA",    "INVALID", "EORA", "ADCA",    "ORAA", "ADDA",    "CPX",  "BSR",  "LDS",  "INVALID", "SUBA", "CMPA", "SBCA",    "INVALID", "ANDA",    "BITA",    "LDAA",    "STAA", "EORA",    "ADCA",    "ORAA", "ADDA",
                                         "CPX",  "INVALID", "LDS",     "STS",     "SUBA",    "CMPA",    "SBCA",    "INVALID", "ANDA",    "BITA",    "LDAA",    "STAA", "EORA",    "ADCA",    "ORAA",    "ADDA", "CPX",  "JSR",     "LDS",     "STS",     "SUBA",    "CMPA",    "SBCA", "INVALID", "ANDA",    "BITA", "LDAA",    "STAA",    "EORA",    "ADCA",    "ORAA",    "ADDA",    "CPX",  "JSR",     "LDS",  "STS",     "SUBB", "CMPB", "SBCB", "INVALID", "ANDB", "BITB", "LDAB",    "INVALID", "EORB",    "ADCB",    "ORAB",    "ADDB", "INVALID", "INVALID", "LDX",  "INVALID",
                                         "SUBB", "CMPB",    "SBCB",    "INVALID", "ANDB",    "BITB",    "LDAB",    "STAB",    "EORB",    "ADCB",    "ORAB",    "ADDB", "INVALID", "INVALID", "LDX",     "STX",  "SUBB", "CMPB",    "SBCB",    "INVALID", "ANDB",    "BITB",    "LDAB", "STAB",    "EORB",    "ADCB", "ORAB",    "ADDB",    "INVALID", "INVALID", "LDX",     "STX",     "SUBB", "CMPB",    "SBCB", "INVALID", "ANDB", "BITB", "LDAB", "STAB",    "EORB", "ADCB", "ORAB",    "ADDB",    "INVALID", "INVALID", "LDX",     "STX"};
  const QList<QString> M6803Mnemonics = {"ZERO", "NOP",  "INVALID", "INVALID", "LSRD", "ASLD", "TAP",  "TPA",  "INX",  "DEX",     "CLV",     "SEV",  "CLC",  "SEC",     "CLI",     "SEI",  "SBA",  "CBA",     "INVALID", "INVALID", "INVALID", "INVALID", "TAB",  "TBA",     "INVALID", "DAA",  "INVALID", "ABA",  "INVALID", "INVALID", "INVALID", "INVALID", "BRA",  "BRN",     "BHI",  "BPL",  "BCC",  "BCS",  "BNE",  "BEQ",     "BVC",  "BVS",  "BPL",     "BMI",     "BGE",  "BLT",     "BGT",     "BLE",  "TSX",  "INS",     "PULA", "PULB",
                                         "DES",  "TCS",  "PSHA",    "PSHB",    "PULX", "RTS",  "ABX",  "RTI",  "PSHX", "MUL",     "WAI",     "SWI",  "NEGA", "INVALID", "INVALID", "COMA", "LSRA", "INVALID", "RORA",    "ASRA",    "ASLA",    "ROLA",    "DECA", "INVALID", "INCA",    "TSTA", "INVALID", "CLRA", "NEGB",    "INVALID", "INVALID", "COMB",    "LSRB", "INVALID", "RORB", "ASRB", "ASLB", "ROLB", "DECB", "INVALID", "INCB", "TSTB", "INVALID", "CLRB",    "NEG",  "INVALID", "INVALID", "COM",  "LSR",  "INVALID", "ROR",  "ASR",
                                         "ASL",  "ROL",  "DEC",     "INVALID", "INC",  "TST",  "JMP",  "CLR",  "NEG",  "INVALID", "INVALID", "COM",  "LSR",  "INVALID", "ROR",     "ASR",  "ASL",  "ROL",     "DEC",     "INVALID", "INC",     "TST",     "JMP",  "CLR",     "SUBA",    "CMPA", "SBCA",    "SUBD", "ANDA",    "BITA",    "LDAA",    "INVALID", "EORA", "ADCA",    "ORAA", "ADDA", "CPX",  "BSR",  "LDS",  "INVALID", "SUBA", "CMPA", "SBCA",    "SUBD",    "ANDA", "BITA",    "LDAA",    "STAA", "EORA", "ADCA",    "ORAA", "ADDA",
                                         "CPX",  "JSR",  "LDS",     "STS",     "SUBA", "CMPA", "SBCA", "SUBD", "ANDA", "BITA",    "LDAA",    "STAA", "EORA", "ADCA",    "ORAA",    "ADDA", "CPX",  "JSR",     "LDS",     "STS",     "SUBA",    "CMPA",    "SBCA", "SUBD",    "ANDA",    "BITA", "LDAA",    "STAA", "EORA",    "ADCA",    "ORAA",    "ADDA",    "CPX",  "JSR",     "LDS",  "STS",  "SUBB", "CMPB", "SBCB", "ADDD",    "ANDB", "BITB", "LDAB",    "INVALID", "EORB", "ADCB",    "ORAB",    "ADDB", "LDD",  "INVALID", "LDX",  "INVALID",
                                         "SUBB", "CMPB", "SBCB",    "ADDD",    "ANDB", "BITB", "LDAB", "STAB", "EORB", "ADCB",    "ORAB",    "ADDB", "LDD",  "STD",     "LDX",     "STX",  "SUBB", "CMPB",    "SBCB",    "ADDD",    "ANDB",    "BITB",    "LDAB", "STAB",    "EORB",    "ADCB", "ORAB",    "ADDB", "LDD",     "STD",     "LDX",     "STX",     "SUBB", "CMPB",    "SBCB", "ADDD", "ANDB", "BITB", "LDAB", "STAB",    "EORB", "ADCB", "ORAB",    "ADDB",    "LDD",  "STD",     "LDX",     "STX"};
